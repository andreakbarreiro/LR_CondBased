 ...
 *    susc = calc_Susc_CondLIF_sigI2(w,E0,sigma,
 ...
 *     ... w.r.t. changes in the I conductance variance (sigI2)
 *
 *  Methods:
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for(i = 0; i < length_w; i++){
	    ...
	    //  For CondLIF: H = tau_m*J + (v-Isyn)*P0
	    //  For CondLIF sigE2: H = tau_m*J + (v-Esyn)*P0/tau_m + (v-Esyn)^2*(dP0/dV)/(2*tau_m)	   
	    dp0_r = (p0_r[j]-p0_r[j-1])/dv;
	    dp0_i = (p0_i[j]-p0_i[j-1])/dv;
	  
	    pe_r[j-1] = pe_r[j]*exp(dv*G[j]) + (tau_m*je_r[j]-(Isyn-v)*rate*p0_r[j]/tau_m + (v-Isyn)*(v-Isyn)*rate*dp0_r/(2*tau_m))*inv_sighatsq[j]*(exp(dv*G[j])-1)/G[j];
            pe_i[j-1] = pe_i[j]*exp(dv*G[j]) + (tau_m*je_i[j]-(Isyn-v)*rate*p0_i[j]/tau_m + (v-Isyn)*(v-Isyn)*rate*dp0_i/(2*tau_m))*inv_sighatsq[j]*(exp(dv*G[j])-1)/G[j];
