 ...
 *	susc = calc_Susc_CondLIF_gI(w,E0,sigma,tau_ref,v_reset,v_th,tau_m,vlb,dv,rate,Esyn,gE,sigE,Isyn,gI,sigI));
 *
 ...
 *     ... w.r.t. changes in the mean INHIBITORY conductance (gI)
 *
 *
 *  Methods:
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for(i = 0; i < length_w; i++){
	    ...
	    pe_r[j-1] = pe_r[j]*exp(dv*G[j]) + (tau_m*je_r[j]-(Isyn-v)*rate*p0_r[j])*inv_sighatsq[j]*(exp(dv*G[j])-1)/G[j];
            pe_i[j-1] = pe_i[j]*exp(dv*G[j]) + (tau_m*je_i[j]-(Isyn-v)*rate*p0_i[j])*inv_sighatsq[j]*(exp(dv*G[j])-1)/G[j];
